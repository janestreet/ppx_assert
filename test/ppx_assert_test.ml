...
let%template _ =
  Int64_u.(([%test_eq: (t box[@kind bits64])] [@alloc a]) { a = 1L + 2L } { a = 3L * 1L })
;;

(* [%test_result] etc should make portable functions *)
let%test_unit _ =
  let _test = [%test_result: int] [@alloc a] in
  let _test = [%test_eq: int] [@alloc a] in
  let _test = [%test_pred: int] [@alloc a] in
  (* ensure this holds for higher-order [compare] functions *)
  let _test = [%test_result: string option] [@alloc a] in
  let _test = [%test_eq: string option] [@alloc a] in
  let _test = [%test_pred: string option] [@alloc a] in
  ()
;;]
